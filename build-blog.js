...
files.forEach(file => {
    ...
    const imageMatch = content.match(/<meta name="article:image" content="(.*?)">/i); // שורה חדשה
    ...
    posts.push({
        ...
        image: imageMatch ? imageMatch[1] : '' // שדה חדש
    });
});

posts.sort((a, b) => new Date(b.date) - new Date(a.date));
fs.writeFileSync(outputFile, JSON.stringify(posts, null, 2));
console.log(`Successfully generated posts.json with ${posts.length} articles, including images.`);
